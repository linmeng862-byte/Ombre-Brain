...
# =============================================================
# Sticky Notes / 小纸条 — Claude 自主给用户留言
# 数据存储在 buckets_dir 下的 .sticky_notes.json
# =============================================================

STICKY_NOTES_FILE = os.path.join(config["buckets_dir"], ".sticky_notes.json")


def _load_sticky_notes() -> list:
    """Load sticky notes from disk."""
...
def _save_sticky_notes(notes: list):
    """Save sticky notes to disk."""
    os.makedirs(os.path.dirname(STICKY_NOTES_FILE), exist_ok=True)
    with open(STICKY_NOTES_FILE, "w", encoding="utf-8") as f:
        f.write(_json_lib.dumps(notes, ensure_ascii=False, indent=2))


@mcp.custom_route("/api/sticky-notes", methods=["GET"])
async def api_sticky_notes_get(request):
...
@mcp.custom_route("/api/sticky-notes", methods=["POST"])
async def api_sticky_notes_create(request):
    """Create a new sticky note manually."""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err: return err
    try:
...
@mcp.custom_route("/api/sticky-notes/{note_id}/read", methods=["POST"])
async def api_sticky_notes_mark_read(request):
    """Mark a sticky note as read."""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err: return err
...
@mcp.custom_route("/api/sticky-notes/{note_id}", methods=["DELETE"])
async def api_sticky_notes_delete(request):
...
@mcp.custom_route("/api/sticky-notes/generate", methods=["POST"])
async def api_sticky_notes_generate(request):
    """Trigger Claude to auto-generate a sticky note based on recent memories (dream + reflect)."""
    from starlette.responses import JSONResponse
    err = _require_auth(request)
    if err: return err
    try:
        ...
        if not recent:
            return JSONResponse({"ok": False, "message": "No recent memories to reflect on"})

        # 2. Build a summary of recent memories
        memory_summary = ""
        for b in recent:
            meta = b["metadata"]
            memory_summary += f"- [{meta.get('domain', '?')}] {b.get('title', '?')} (情绪: {meta.get('valence', 0):.1f}/{meta.get('arousal', 0):.1f})\n"

        # 3. Ask the LLM to write a caring sticky note
        prompt = (
            "你是一个温暖、体贴的 AI 伙伴。根据以下最近的记忆，写一张「小纸条」给用户。\n"
            "要求：\n"
            "- 语气温柔关心，像好朋友写的便利贴\n"
            "- 1-3 句话，简短有温度\n"
            "- 可以是鼓励、提醒、关心或小感悟\n"
        ...
        # Call the dehydration LLM to generate the note
        dehy_config = config.get("dehydration", {})
        api_key = dehy_config.get("api_key", "") or os.environ.get("OMBRE_API_KEY", "")
        if not api_key:
        ...
        logger.error(f"Sticky note generation failed: {e}")
        return JSONResponse({"error": str(e)}, status_code=500)


# --- Auto-generate sticky notes periodically ---
# 自动定时生成小纸条（每 6 小时）
STICKY_NOTE_INTERVAL_HOURS = int(os.environ.get("OMBRE_STICKY_NOTE_INTERVAL", "6"))


async def _auto_sticky_note_loop():
    """Periodically auto-generate sticky notes from dream reflection."""
    ...
    while True:
        try:
            ...
            if not recent:
                logger.debug("Auto sticky note: no recent memories, skipping")
                continue
            ...
            notes.append(note)
            _save_sticky_notes(notes)
            logger.info(f"Auto sticky note generated: {note_content[:50]}...")
        except Exception as e:
            logger.warning(f"Auto sticky note generation failed: {e}")
